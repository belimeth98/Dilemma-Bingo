...
## 참여자 표시와 퇴장 데이터 자동 정리

게임 현황의 참여자 목록은 현재 접속 중인 참여자를 최대 6명까지 표시합니다. 넓은 화면의 카드는 3열×2행, 좁은 화면은 2열×3행으로 표시해 기존 높이 제한에 가려지지 않도록 했습니다. 6명을 넘으면 추가 접속 인원 수를 안내합니다. `접속 대기` 참여자는 공개 참여자 응답·목록과 승리 임박 안내에서 제외합니다. 과거 공개 로그는 그대로 유지합니다.

`cleanup.py`의 정리 작업은 FastAPI 서버 시작 시 한 번, 이후 **24시간마다** 실행됩니다. 서버 종료 시 작업을 취소하고 종료를 기다린 뒤 DB 연결을 정리합니다. 별도의 패키지, OS 작업 스케줄러 또는 외부 서비스는 필요하지 않습니다.

- 삭제 조건: `room_players.connected = false`이고, **명시적으로 '나가기'를 눌러 기록된 `left_at`으로부터 24시간 이상 경과**한 참여자.
- 삭제 대상: 해당 방의 참여자 행(닉네임, 빙고판, 마킹, 방 내 누적 점수 등)과 대응하는 메모리 참여자/점수 정보.
- 보존 대상: 방·게임 자체, 다른 방에서 같은 사용자의 정보, 과거 `game_results`, 공개 로그. 브라우저 종료·네트워크 단절 등 `left_at`이 없는 일시적인 연결 끊김도 재접속을 위해 보존합니다.
- 재접속·현재 연결·진행 중인 거래/보너스 대상은 삭제 직전에 다시 확인하여 보호합니다. 재접속하면 기존 저장 로직이 `left_at`을 초기화하므로 과거 퇴장 시각으로 삭제되지 않습니다.
- 기존 방별 lock 및 DB 행 lock을 이용합니다. DB 커밋 성공 후에만 메모리 캐시를 정리하고, 실패하면 롤백 후 다음 점검에서 재시도합니다.
- 한 번에 최대 500개 후보를 처리합니다. 일반적으로 퇴장 후 24시간이 지난 다음 24시간 단위 점검에서 삭제하며, 서버 중단·DB 장애·대량 후보 등으로 지연되면 이후 점검에서 처리합니다.

기존 `left_at`, `connected` 컬럼을 사용하므로 **스키마 변경이나 migration은 없습니다. 단, 서버 실행 후 위 조건에 맞는 참여자 데이터의 실제 DELETE가 수행됩니다.** 삭제 후 다시 입장한 참여자는 해당 방에서 새 참여자로 시작하며, 이전 방 내 점수/빙고판은 복구되지 않습니다. 기존 단일 서버 프로세스 운영 전제를 유지합니다.


## 관전 중 종료된 게임의 결과 보관

관전 화면에 표시된 게임은 종료되더라도 자동으로 목록에서 제거하지 않습니다. 기존 `games.status = ENDED`와 `game_results.won` 기록을 조회하여 **게임 종료**, **우승자(공동 우승 포함)** 및 **X 닫기 버튼**을 표시합니다. 결과 카드도 기존 1/2/4개 그리드와 최신 시작순 페이지네이션에 포함됩니다.

- X는 해당 관전자 탭의 목록에서 결과를 닫습니다. 서버의 게임 결과를 삭제하거나 다른 관전자 화면을 변경하지 않습니다.
- 관전한 게임 ID는 탭별 `sessionStorage`에 보관합니다. 같은 탭의 새로고침·WebSocket 재접속 후에도 결과와 닫기 상태가 유지됩니다. 탭을 닫아 저장 영역이 사라지면 해당 탭의 관전 목록은 초기화되며, 저장소를 사용할 수 없는 브라우저에서는 현재 화면 메모리에서만 유지됩니다.
- 동일한 방에서 다음 게임이 시작되어도 게임 ID로 구분하므로 이전 우승 결과가 덮어써지지 않습니다.
- 우승자 이름은 당시 게임 결과에서 읽으므로 닉네임 변경, 서버 재시작, 퇴장 참여자 정리 후에도 결과가 유지됩니다. 이전에 관전하지 않았던 모든 과거 게임을 새 관전자에게 나열하지는 않습니다.
- `/api/game-status/ws` 조회 조건에 `watched_game_ids`를 추가했습니다. 서버는 직전에 전달한 게임도 자동 추적하여 다음 갱신 전에 게임이 끝나는 경우에도 결과를 조회합니다. 빙고판·마킹·재접속 식별값은 계속 비공개입니다.
- 진행 게임의 플레이 로직과 저장 방식은 변경하지 않았으며, DB 컬럼 추가나 migration도 필요하지 않습니다.

정리 스케줄러는 서버 시작 시 1회 실행한 뒤 24시간 간격으로 점검합니다. 삭제 자격은 기존과 동일하게 명시적 퇴장 후 24시간 경과이며, 주기 변경으로 실제 삭제는 정상 운영 시 대체로 퇴장 후 24~48시간 사이의 다음 점검에서 수행됩니다. 장애·대량 후보·거래 보호 등에 따른 지연은 별도입니다.
