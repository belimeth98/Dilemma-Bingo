...
`cleanup.py`의 정리 작업은 FastAPI 서버 시작 시 한 번, 이후 **10분마다** 실행됩니다. 서버 종료 시 작업을 취소하고 종료를 기다린 뒤 DB 연결을 정리합니다. 별도의 패키지, OS 작업 스케줄러 또는 외부 서비스는 필요하지 않습니다.
...
- 한 번에 최대 500개 후보를 처리합니다. 일반적으로 퇴장 후 24시간이 지난 다음 10분 단위 점검에서 삭제하며, 서버 중단·DB 장애·대량 후보 등으로 지연되면 이후 점검에서 처리합니다.
